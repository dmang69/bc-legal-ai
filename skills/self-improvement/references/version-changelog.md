# Version Changelog — All Skills

## bc-legal-ai suite

| Skill | Current | Last change |
|-------|---------|-------------|
| supreme-court-civil-counsel | 1.1 | SRL + multi-discipline role |
| bc-tenancy-substantive | 1.0 deep | Modules 1–7 RTA mastery |
| bc-tenancy-procedure | 1.0 runtime | Evidence → enforcement |
| bc-tenancy-advocacy / advanced | 1.0 | Modules 1–7 advanced |
| bc-judicial-review-guide | 1.0 | JR handoff + stay framing |
| canlii-boa-builder | 0.1 scaffold | BOA discipline |
| self-improvement | 1.0 | Meta-learning framework |
| learning-mode | 1.0 | Tutoring pedagogy |
| doc-coauthoring | 1.0 | Collaborative doc workflow |
| cognitive-awareness | 1.0 | Proactive metacognition companion |
| cognitive-awareness | 1.0.1 | Module 6 case studies + synthesis library + quality gate |
| cognitive-awareness | 1.0.2 | Confidence calibration worked examples; RTA s.6 false-cite trap |
| cognitive-awareness | 1.0.3 | Expanded failure-mode catalog (10 modes + antidotes + meta) |
| cognitive-awareness | 1.0.4 | Expanded situational signals vocabulary (Module 1) |

## Log

### 2026-06-20 — self-improvement v1.0
- Added: full Modules 1–10, templates, references  
- Reason: formal meta-learning layer for suite  
- Source: user specification  
